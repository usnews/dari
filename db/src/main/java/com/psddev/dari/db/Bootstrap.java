...
public interface Bootstrap {
    ...
    @ObjectType.AnnotationProcessorClass(PackagesProcessor.class)
    ...
    @ObjectType.AnnotationProcessorClass(PackageProcessor.class)
    @Retention(RetentionPolicy.RUNTIME)
    @Target(ElementType.TYPE)
    ...
    @TypeModification.FieldInternalNamePrefix("bootstrap.")
    ...
    public final static class Static {
        ...
        private static Map<String, BootstrapPackage> getPackagesMap(Database database) {
            ...
            for (ObjectType type : database.getEnvironment().getTypes()) {
                TypeModification bmod = type.as(TypeModification.class);
...
class PackageProcessor implements ObjectType.AnnotationProcessor<Bootstrap.Package> {
    @Override
    public void process(ObjectType type, Bootstrap.Package annotation) {
        Set<String> packageNames = new HashSet<String>();
        packageNames.add(annotation.value());
        type.as(Bootstrap.TypeModification.class).setPackageNames(packageNames);
    }
}

class PackagesProcessor implements ObjectType.AnnotationProcessor<Bootstrap.Packages> {
    @Override
    public void process(ObjectType type, Bootstrap.Packages annotation) {
        Set<String> packageNames = new HashSet<String>(Arrays.asList(annotation.value()));
        type.as(Bootstrap.TypeModification.class).setPackageNames(packageNames);
    }
}
