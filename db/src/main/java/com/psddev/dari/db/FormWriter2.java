...
public class FormWriter2 extends HtmlWriter {

    private FormLabelRenderer labelRenderer;
    private FormInputProcessor2 defaultInputProcessor;
    private Map<String, FormInputProcessor2> inputProcessors;

    /** Creates an instance that writes to the given {@code writer}. */
    public FormWriter2(Writer writer) {
        super(writer);
    }
     ...
     * @return Can't be {@code null}.
     */
    public FormInputProcessor2 getDefaultInputProcessor() {
        if (defaultInputProcessor == null) {
            setDefaultInputProcessor(new FormInputProcessor2.Default());
        }
        return defaultInputProcessor;
    }

    /** Sets the default form input processor. */
    public void setDefaultInputProcessor(FormInputProcessor2 defaultInputProcessor) {
     ...
     * @return Can't be {@code null}.
     */
    public Map<String, FormInputProcessor2> getInputProcessors() {
        if (inputProcessors == null) {
            setInputProcessors(new HashMap<String, FormInputProcessor2>());
        }
        return inputProcessors;
    }

    /** Sets the map of all input processors. */
    public void setInputProcessors(Map<String, FormInputProcessor2> inputProcessors) {
        this.inputProcessors = inputProcessors;
    }

    /** Puts all standard input processors. */
    public void putAllStandardInputProcessors() {
        Map<String, FormInputProcessor2> inputProcessors = getInputProcessors();
        ...
        inputProcessors.put(ObjectField.RECORD_TYPE, new FormInputProcessor2.ForRecord(this));
    ...
    protected FormInputProcessor2 findInputProcessor(ObjectField field) {
        String type = field.getInternalType();

        if (type != null) {
            Map<String, FormInputProcessor2> processors = getInputProcessors();
     ...
     * and {@link #allInputs}.
     */
    protected void writeField(State state, ObjectField field, FormInputProcessor2 processor) throws IOException {
     ...
     */
    protected void updateField(State state, HttpServletRequest request, ObjectField field, FormInputProcessor2 processor) {
        String fieldName = field.getInternalName();
        String inputName = state.getId() + "/" + fieldName;
